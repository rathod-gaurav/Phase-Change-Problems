...
void AllenCahn<Nsd,BfOrder>::assemble_system_B(){
    ...
    FullMatrix<double> dBlocal_dphi(dofs_per_cell, dofs_per_cell);
    ...
    for(const typename DoFHandler<Nsd>::active_cell_iterator &elem : dof_handler.active_cell_iterators()){
        ...
        compute_element_B(elem, fe_values, Blocal, dBlocal_dphi, local_dof_indices);

        for(const unsigned int i : fe_values.dof_indices()){
            Bglobal(local_dof_indices[i]) += Blocal(i);
            for(const unsigned int j : fe_values.dof_indices()){
                dBglobal_dphi.add(local_dof_indices[i], local_dof_indices[j], dBlocal_dphi(i,j));
            }
        }
    }
